...
<div class="ad-shell ta-shell">
    <header class="ad-topbar">
        <div class="brand-group">
            ...
            <div>
                <div class="brand-title">Edit Position</div>
                <div class="brand-subtitle">Update job information</div>
            </div>
        </div>
    ...
    <div class="ad-layout ta-layout">
        ...
        <main class="ad-main">
            ...
            <section class="page-head">
                <div>
                    <h1 style="font-size:38px;">Edit Position</h1>
                    <p>Update job details</p>
                </div>
            </section>

            <section class="list-card" style="margin-top:14px;padding:20px;">
                <form action="${pageContext.request.contextPath}/mo/edit" method="post" style="display:grid;gap:16px;max-width:600px;">
    <input type="hidden" name="jobId" value="<%= job.getJobId() %>" />
    <input type="hidden" name="status" value="<%= job.getStatus() %>" />
    <input type="hidden" name="publishedAt" value="<%= job.getPublishedAt() %>" />

    <label class="filter-field">
        <small>MODULE NAME</small>
        <input type="text" name="moduleName" value="<%= job.getModuleName() %>" required style="width:100%;padding:8px;" />
    </label>
    <label class="filter-field">
        <small>JOB TYPE</small>
        <input type="text" name="jobType" value="<%= job.getJobType() %>" required style="width:100%;padding:8px;" />
    </label>
    <label class="filter-field">
        <small>INTRODUCTION (Job Description)</small>
        <textarea name="introduction" rows="3" required style="width:100%;padding:8px;"><%= job.getIntroduction() %></textarea>
    </label>
    <label class="filter-field">
        <small>REQUIREMENTS (Qualifications)</small>
        <textarea name="requirements" rows="3" required style="width:100%;padding:8px;"><%= job.getRequirements() %></textarea>
    </label>
    <label class="filter-field">
        <small>WEEKLY WORKLOAD (HOURS)</small>
        <input type="number" name="weeklyWorkload" value="<%= job.getWeeklyWorkload() %>" min="1" required style="width:100%;padding:8px;" />
    </label>
    <label class="filter-field">
        <small>DEADLINE</small>
        <input type="date" name="deadline" value="<%= job.getDeadline() %>" required style="width:100%;padding:8px;" />
    </label>

    <!-- 新增：回显招聘人数（仅新增这段，其余不动） -->
    <label class="filter-field">
        <small>RECRUIT LEADER COUNT</small>
        <input type="number" name="leaderCount" min="0" value="<%= job.getLeaderCount() %>" required style="width:100%;padding:8px;" />
    </label>
    <label class="filter-field">
        <small>RECRUIT MEMBER COUNT</small>
        <input type="number" name="memberCount" min="0" value="<%= job.getMemberCount() %>" required style="width:100%;padding:8px;" />
    </label>

    <div>
        <button type="submit" class="chip-button active">Save Changes</button>
    </div>
</form>
            </section>
        </main>
